...
# Load your dataset
dat.180 <- read_excel("DC_FEF_Main.xlsx", sheet = "180data")
...
sidebar <- dashboardSidebar(
  sidebarMenu(
    ...
    sidebarMenu(
      checkboxGroupInput("watershed_selector", "Select Watersheds", choices = unique(dat.180$watershed)),
      checkboxGroupInput("variable_selector", "Select Variables", choices = names(dat.180)[8:ncol(dat.180)]),
      checkboxGroupInput("depth_selector", "Select Depth", choices = unique(dat.180$depth)),
  ...
  tabItems(
    tabItem(
      ...
      conditionalPanel(
        ...
        HTML("<p style='color: black; text-align: center; font-size: 16px; font-weight: bold;'>Please select at least one watershed, one carbon fraction, and one depth to use the explorer tool.</p>")
      ),
      fluidRow(
        column(6, plotlyOutput("carbon_plot")),
        column(6, 
...
ui <- dashboardPage(
  dashboardHeader(title = "Soil Carbon Explorer"),
  sidebar,
  ui
)
